...
DoorItem::DoorItem(Point* spawn) : Item(spawn) {
	...
	this->height = 32;
	this->is_locked = true;
	std::vector<std::string> animations = { "sword_animation" };
	this->animator = new PlayerAnimator("Engine/Configs/item/SwordAnimatorConfig.json", 0, animations);

}
void DoorItem::PickUp(Player& player) {
	std::cout << "door collison!\n";
	if (player.UseKey())
		this->Unlock();
}
